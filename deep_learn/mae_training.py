    ...
    def __getitem__(self, idx):
        ...
        if self.from_npy:
            ...
            if os.path.exists(npy_path + ".npy"):
                ...
                print(f"File not found: {npy_path}.npy")
        else:
            x = self.data[idx]
        ...
        if self.method == "cola":
            ...
            if p < 0:
                ...
                #changed this from random crop because most delay data is in the first section of the image
            elif p >0:
...
def mae_train_multiple_data(title, data_source={"covidbreath": 251}, dim_hidden=1280, dim_out=512, encoder="vit", n_epoches=150, training_method='mae'):
    ...
    for dt, max_len in data_source.items():
        if dt in ['covidbreath', 'covidcough']:
            modality = dt[5:]
            filenames = list(np.load("datasets/covid19-sounds/SSL_entireaudio_filenames_8000_{}.npy".format(modality)))

        elif dt == "icbhi":
            icbhi_filenames = np.load("datasets/icbhi/entire_spec_filenames_8000.npy")
...
if __name__ == "__main__":
    ...
    # optimal_max_len = {"covidbreath": 64, "covidcough": 64, "icbhicycle": 64,  "coughvid":64, "hf_lung":64, "covidUKexhalation": 64, "covidUKcough": 64} #equal length traning
    optimal_max_len = {"covidbreath": 32, "covidcough": 32, "icbhicycle": 32,  "coughvid":32, "hf_lung":32, "covidUKexhalation": 32, "covidUKcough": 32} #equal length traning
    data_source = {}
