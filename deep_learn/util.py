...
def slice(spectrogram, dt):
    
    snippets = []
    t = 0

    while t+dt <= spectrogram.shape[1]:

        snippets.append(spectrogram[:,t:t+dt])
        t += dt
...
def delay_img(spectrogram):
        
    #dt = int(np.round(sample_rate*time_slice/hop))

    #dt = 1

    #snippets = slice(spectrogram, dt)
    length = spectrogram.shape[1]

    # Efficiently compute the delay matrix using broadcasting
    delay_matrix = np.abs(spectrogram[:, :, None] - spectrogram[:, None, :])

    #print(delay_matrix.shape)

    delay_t = np.zeros((length,64))
    offset = 0
    for i in range(length):
        diagonal = np.array([np.diagonal(matrix, offset= i) for matrix in delay_matrix])
        delay_t[i] = np.mean(diagonal,axis=1)

    #print(np.array(delay_t).shape)
...
def get_entire_signal_librosa(data_folder, filename, input_sec=8, sample_rate=16000, butterworth_filter=None, spectrogram=False, pad=False, from_cycle=False, yt=None, types='repeat'):
    ...
    if duration < input_sec:
        if not pad:
            ...
            yt = split_pad_sample([yt, 0,0], input_sec, sample_rate, types = 'zero')[0][0]
    ...
    if spectrogram:
        ...
        spec = pre_process_audio_mel_t(yt.squeeze(), f_max=8000)

        delay_spec = delay_img(spec.T)
        #print('yay')
        return delay_spec
